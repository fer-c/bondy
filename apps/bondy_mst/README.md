...
`get_range/2` · `fold/3,4` · `fold_pages/4` · `foreach/2,3` · `root/1` ·
`capabilities/1`
...
page-level key-value contract (`open`, `get_root`/`set_root`, `get`, `put`,
`has`, `delete`, `free`, `gc`, `missing_set`, `page_refs`, `list`, `destroy`,
and optional `transaction`/`capabilities`). GC is **mark-from-root**: keep only
