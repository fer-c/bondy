...
send(RealmUri, To, Msg, Opts) ->
    ...
    bondy_wamp_message:is_message(Msg) orelse
        error({invalid_wamp_message, Msg}),
...
set_process_metadata(Meta0, LogKeys0) when is_map(Meta0), is_list(LogKeys0) ->
    {LogKeys, Meta} =
        case maps:take('$log_keys', Meta0) of
            {LogKeys1, Meta1} ->
                L = sets:to_list(
                    sets:union(
                        sets:from_list(LogKeys0), sets:from_list(LogKeys1)
                    )
                ),
                {L, Meta1};
            error ->
...
update_process_metadata(Meta0, LogKeys0) when
    is_map(Meta0), is_list(LogKeys0)
->
    {LogKeys, Meta} =
        case maps:take('$log_keys', Meta0) of
            {LogKeys1, Meta1} ->
...
check_response(Uri, ReqId, Timeout, Ctxt) ->
    receive
        {?BONDY_REQ, _Pid, _RealmUri, #result{} = R} when
            R#result.request_id == ReqId
        ->
            %% ok = bondy:ack(Pid, Ref),
            {ok, message_to_map(R)};
        {?BONDY_REQ, _Pid, _RealmUri, #error{} = R} when
            R#error.request_id == ReqId
        ->
            %% ok = bondy:ack(Pid, Ref),
