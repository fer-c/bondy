...
map({missing_required_value, Key}) ->
    #{
        ...
        <<"description">> =>
            <<"A value for ", $', (to_bin(Key))/binary, $', " is required.">>,
        <<"key">> => Key
    };
...
map({no_such_realm, Uri}) ->
    #{
        ...
        <<"description">> =>
            <<"A realm named ", $', Uri/binary, $', "could not be found.">>
    };
...
map({badarg, {body_max_bytes_exceeded, MaxLen}}) ->
    #{
        ...
        <<"message">> =>
            <<"The body content size exceeds the allowable limit of", $\s,
                (integer_to_binary(MaxLen))/binary, $\s, "bytes">>,
        <<"description">> =>
            <<"The body cannot be larger that the defined maximum allowed.">>
    };
...
map({badheader, Header, Desc}) when
    is_binary(Desc); is_list(Desc); is_atom(Desc)
->
    #{
        <<"code">> => invalid_argument,
