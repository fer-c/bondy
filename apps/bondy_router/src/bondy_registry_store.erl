...
new(PartitionIndex) ->
    ...
    R2 = bondy_registry_ptrie:new(
        gen_name(reg_prefix_idx_ptrie, PartitionIndex)
    ),
    ...
    S3 = bondy_registry_ptrie:new(
        gen_name(sub_prefix_idx_ptrie, PartitionIndex)
    ),
...
match_wildcard(Store, Type, RealmUri, Uri, Opts) when
    Type == registration
->
...
This call executes concurrently.
""".
-spec find_exact_matches(t(), entry_type(), uri(), uri(), map()) ->
    reg_match_result().
...
find_prefix_matches(Store, Type, RealmUri, Uri, Opts) when
    Type == registration
->
    Ptrie = Store#bondy_registry_store.reg_prefix_idx_ptrie,
...
find_wildcard_matches(Store, Type, RealmUri, Uri, Opts) when
    Type == registration
->
    Ptrie = Store#bondy_registry_store.reg_wc_idx_ptrie,
    ptrie_find_matches(Store, Type, RealmUri, Uri, Opts, Ptrie);
...
validate_entry(Entry) ->
    bondy_registry_entry:is_entry(Entry) orelse
        ?ERROR(badarg, [Entry], #{
...
%% @private
validate_key(Entry) ->
    bondy_registry_entry:is_key(Entry) orelse
        ?ERROR(badarg, [Entry], #{
...
ms_id_set_filter(Key, Opts) ->
    case maps:find(Key, Opts) of
        error -> '_';
        {ok, []} -> '_';
        {ok, Ids} -> sets:from_list([integer_to_binary(I) || I <- Ids])
    end.
...
keep_subscription(Ns, ProtoSessId, MyNs, NodeFilter, Eligible, Exclude) ->
    case
        {Ns =/= MyNs andalso is_binary(Ns), node_filter_match(Ns, NodeFilter)}
    of
        {_, false} ->
            skip;
        {true, true} ->
...
merge_results([L | T], registration = Type, FN, Acc) when
    is_list(L), is_list(Acc)
->
    merge_results(T, Type, FN, lists:append(L, Acc));
merge_results([{L1, C1} | T], registration = Type, FN, {L0, C0}) when
    is_list(L0), is_list(L1)
->
    C = merge_continuation(FN, [C1, C0]),
    merge_results(T, Type, FN, {lists:append(L1, L0), C});
merge_results([{L1, R1} | T], subscription = Type, FN, {L0, R0}) when
    is_list(L0), is_list(R0), is_list(L1), is_list(R1)
->
    %% The case for subscriptions w/o limit
    Acc = {lists:append(L1, L0), lists:append(R1, R0)},
    merge_results(T, Type, FN, Acc);
merge_results([{{L1, R1}, C1} | T], subscription = Type, FN, {{L0, R0}, C0}) when
    is_list(L0), is_list(R0), is_list(L1), is_list(R1)
->
...
%% @private
match_each(#bondy_registry_store{} = Store, Type, RealmUri, Uri, Opts, [H | T]) when
    H == ?EXACT_MATCH
->
    case match_exact(Store, Type, RealmUri, Uri, Opts) of
        ?EOT ->
...
match_each(#bondy_registry_store{} = Store, Type, RealmUri, Uri, Opts, [H]) when
    H == ?WILDCARD_MATCH
->
    match_wildcard(Store, Type, RealmUri, Uri, Opts);
...
ptrie_match(Store, Type, RealmUri, Uri, Opts, Ptrie, Policy) ->
    ...
    Entries =
        case bondy_registry_ptrie:lookup(Ptrie, Key, Policy) of
