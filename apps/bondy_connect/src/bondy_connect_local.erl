...
`bondy_connect` standalone.

`bondy_connect` must compile and run on a *peer* node that does **not** bundle
...
The dependency direction is one-way: **`bondy → bondy_connect`** (to implement
the behaviour and register). `bondy_connect` never names a `bondy` module.
...
`{$bondy_request, …}` protocol detail never leaks into `bondy_connect`. There is
