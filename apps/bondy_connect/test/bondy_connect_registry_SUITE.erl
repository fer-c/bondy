...
declare_then_confirm_registration(_) ->
    ...
    R1 = bondy_connect_registry:declare_registration(<<"a.b">>, H, #{x => 1}, R0),
    ...
    ?assertEqual({ok, 99}, bondy_connect_registry:registration_id(<<"a.b">>, R2)).
...
declare_then_confirm_subscription(_) ->
    ...
    ?assertEqual({ok, 7}, bondy_connect_registry:subscription_id(<<"t.1">>, R2)).
...
lookup_by_id_and_uri(_) ->
    ...
    ?assertEqual({ok, 10}, bondy_connect_registry:registration_id(<<"p.1">>, R)),
    ?assertEqual({ok, 20}, bondy_connect_registry:registration_id(<<"p.2">>, R)),
...
declared_lists(_) ->
    H = fun(_, _, _) -> ok end,
    R0 = bondy_connect_registry:new(),
    R1 = bondy_connect_registry:declare_registration(<<"p.1">>, H, #{a => 1}, R0),
    R2 = bondy_connect_registry:declare_subscription(<<"t.1">>, H, #{b => 2}, R1),
    ?assertEqual(
        [{<<"p.1">>, H, #{a => 1}}],
...
clear_established_keeps_declared(_) ->
    ...
    R3 = bondy_connect_registry:declare_subscription(<<"t.1">>, H, #{b => 2}, R2),
    ...
    ?assertEqual({ok, 11}, bondy_connect_registry:registration_id(<<"p.1">>, R5)).
