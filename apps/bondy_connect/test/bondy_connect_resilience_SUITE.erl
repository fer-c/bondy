...
revocation_keeps_declared_replays_on_reconnect(_) ->
    ...
    {Callee, CalleeServer} = connect_and_server(#{}),
    {ok, RegId} = bondy_connect:register(Callee, Proc, echo_handler()),
    ...
    _ = exit(CalleeServer, kill),
    ok = wait_until(
        fun() -> bondy_connect:status(Callee) =:= established end, 200, 100
    ),
    ok = wait_until(
        fun() ->
            case bondy_connect:call(Caller, Proc, [<<"c">>]) of
                {ok, #{args := [<<"c">>]}} -> true;
                _ -> false
            end
        end,
