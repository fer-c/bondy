...
validate_params(#{kdf := pbkdf2, iterations := N} = Params)
when is_integer(N) andalso N >= 4096 andalso N =< 65536 ->
    Static = #{
...
-spec salted_password(
    Password :: binary(), Salt :: binary(), Params :: params()) -> binary().
...
""".
-spec response(
    Challenge :: binary(), Password :: binary(), Params :: map()) -> binary().
